...
import bd.gov.dgfood.common.services.service.persistence.AttachmentUtil;
 ...
 * @generated
 */
@Component(service = {AttachmentPersistence.class, BasePersistence.class})
public class AttachmentPersistenceImpl
	 ...
	 */
	@Override
	public List<Attachment> findByCPK_CN(long classPK, String className) {
		return findByCPK_CN(
	 ...
	 */
	@Override
	public List<Attachment> findByCPK_CN(
		...
		if (useFinderCache) {
			...
			if ((list != null) && !list.isEmpty()) {
				for (Attachment attachment : list) {
					if ((classPK != attachment.getClassPK()) ||
						!className.equals(attachment.getClassName())) {
	...
	public Attachment findByCPK_CN_First(
		...
		Attachment attachment = fetchByCPK_CN_First(
	...
	public Attachment fetchByCPK_CN_First(
		...
		List<Attachment> list = findByCPK_CN(
	...
	public Attachment findByCPK_CN_Last(
		...
		Attachment attachment = fetchByCPK_CN_Last(
			classPK, className, orderByComparator);
	...
	public Attachment[] findByCPK_CN_PrevAndNext(
		...
		try {
			...
			array[0] = getByCPK_CN_PrevAndNext(
				session, attachment, classPK, className, orderByComparator,
				true);
	...
	protected Attachment getByCPK_CN_PrevAndNext(
		Session session, Attachment attachment, long classPK, String className,
	...
	private static final String _FINDER_COLUMN_CPK_CN_CLASSNAME_3 =
		"(attachment.className IS NULL OR attachment.className = '')";
